...
public class SiteContentsPage {
	...
	public void switchToFrame() {
		try {
			Testfactory.switchToIframe(
					(Testfactory.driver
							.findElements(By.xpath("//a[contains(text(), '" + ExcelUtils.getSheetData(9, 1) + "')]"))),
					0);
		} catch (Exception e) {
	...
	public void clickOnLibrary() {
		try {
			Testfactory.clickAction(Testfactory.driver
					.findElement(By.xpath("//a[contains(text(), '" + ExcelUtils.getSheetData(9, 1) + "')]")));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
